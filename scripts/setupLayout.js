...
define([
    ...
    function setupLayout() {
        ...
        function setupAirspaceCalculator() {
            wsdot.airspaceCalculator = new AirspaceCalculatorArcGisUI("http://hqolymgis99t/arcgis/rest/services/Airport/Airport_Surfaces_40ft_Int/ImageServer");
                    ...
                    function createZoomControl(qtName, data) {
                        ...
                        if (data.url) {
                            ...
                            queryTask.task.execute(queryTask.query, function (featureSet) {
                                createExtentSelect(selectName, featureSet, wsdot.map, data.levelOrFactor);
                            });
                        } else if (data.extents) {
                            createExtentSelect($("<div>").attr("id", selectName).appendTo(cell)[0], data.extents, wsdot.map);
