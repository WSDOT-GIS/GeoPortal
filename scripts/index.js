...
function init() {
    ...
    * @param {esri.tasks.FeatureSet} featureSet A set of features returned from ArcGIS Server.
    * @param {string} name The name of the feature attribute that will be used to label items in the autocomplete dropdown.
    ...
    */
    function setupZoomControl(inputControl, featureSet, nameAttribute, mapToZoom) {
        var graphic;
        var name;
        var option
        var extents = [];
    ...
    cityQueryTask.execute(query, function (featureSet) {
        setupZoomControl("#cityZoomSelect", featureSet, "NAME", map);
    });
    ...
    urbanAreaQueryTask.execute(query, function (featureSet) {
        setupZoomControl("#urbanAreaZoomSelect", featureSet, "NAME", map);
    });
}
