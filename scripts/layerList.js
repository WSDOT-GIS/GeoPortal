...
require([
  ...
  function makeIdSafeString(s, replacement, prefix, alwaysUsePrefix) {
    ...
    if (alwaysUsePrefix || /^[^a-z]/i.test(s)) {
      // JSLint will complain about this Regex's "insecure ^", but this is not used for security purposes so it should be fine.
      s = [prefix || "z-", s].join("");
    }
  ...
  function getLayerConstructor(layerType) {
    ...
    if (typeof layerType === "string") {
      if (/(?:esri\.layers\.)?ArcGISTiledMapServiceLayer/i.test(layerType)) {
        ...
        /(?:esri\.layers\.)?ArcGISImageServiceLayer/i.test(layerType)
      ) {
  ...
  function setOpacity(event, ui) {
    var value = event.target.valueAsNumber || ui.value,
  ...
  onLayerLoad = function(layer) {
    ...
    // Add options link
    tools = $(this.options.contextMenuIcon)
      .appendTo($element)
      .click(
        {
          layer: layer
  ...
  $.widget("ui.layerListItem", {
    ...
    setIsInScale: function(scale) {
      var layer,
        ...
        maxScale,
        isInScale,
        outOfScaleClass = "ui-layer-list-out-of-scale";
  ...
  $.widget("ui.layerListGroup", {
    ...
    _create: function() {
      ...
      link = $(["<a href='#'>", $this.options.groupName, "</a>"].join(""))
        .attr("href", "#")
        .appendTo($this.element);
  ...
  $.widget("ui.layerList", {
    ...
    _addLayer: function(layer, error) {
      ...
      if (!error && !this._layerExistsInToc(layer)) {
        ...
        layerListItem = $(this._childNodeType)
          .appendTo(parent)
          .layerListItem({
            layer: layer,
            map: this.options.map,
    ...
    _removeLayer: function(layer) {
      ...
      listItems = $(".ui-layer-list-item").filter(
    ...
    _addLayersAlreadyInMap: function() {
      var i,
        l,
        map = this.options.map,
        layerIds = map.layerIds.concat(map.graphicsLayerIds);
    ...
    _create: function() {
      ...
      $this._childNodeType = /[uo]l/i.test(baseNode.nodeName)
        ? "<li>"
        : "<div>";
